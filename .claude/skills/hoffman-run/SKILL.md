...
#$ -m ea
```
in your job scripts so the user gets email notifications on job start and end.
- Be sure to keep a copy of each submit script alongside the outputs for an audit trail. Include a command similar to this if the script outputs to a certain folder:
`cp "$(realpath "$0")" "$OUTPUT_BASE/$(basename "$0")"`
- Logs should be sent to one of the subfolders of `/u/scratch/a/aliceg/logs`. You should speify the path of the log file in the `#$ -o` line of your job script. For example, you could use `#$ -o /u/scratch/a/aliceg/logs/misc/$JOB_ID.$TASK_ID.out`.
