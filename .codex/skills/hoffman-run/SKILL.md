...
- **A vanished job isn't a passed job** — verify with `qacct` (see "Did it actually succeed?"). An `Eqw` job usually needs **`qdel -f`** to clear.
- **`$SCRATCH` is purged periodically** — don't treat it as permanent storage.
- **Slow `highp` start?** If a `highp` CPU job sits queued >5–15 min, resubmit without `highp`.
